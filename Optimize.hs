...
type Stack = [(StackEntry,Int)]
data StackEntry = Undefined | Unknown Register | Known Integer
    deriving (Show)
 
stateToEntry (x:xs) = (x,xs)
stateToEntry [] = ((Undefined,-1),[])
pop :: State Stack (StackEntry,Int)  
pop = state $ stateToEntry
...
run_stack_analysis insts=
    let (instructions,indices) = fst $ runState (stack_analysis insts 0) []
        sorted = sort indices
